...
main_config = {}
problem_config = {}
prompt_context_parts = {} # Renamed to avoid conflict with prompt_parts in llm_generator
llm_config = {}

# --- Prompt Templating Constants ---
DEFAULT_PROMPT_TEMPLATE_NAME = "prompt_generation_prompt.jinja"
HIERARCHICAL_ORCHESTRATOR_TEMPLATE_NAME = "hierarchical_prompt_generation_prompt.jinja"
DELEGATED_SUBTASK_TEMPLATE_NAME = "delegated_prompt_subtask_prompt.jinja"

# --- Configuration Loading ---
MAIN_CONFIG_FILE = "config/config.yaml"
PROBLEM_CONFIG_FILENAME = "problem_config.yaml"
PROMPT_CONTEXT_FILENAME = "prompt_context.yaml" # This is for problem-specific context for prompt generation

def load_all_configs():
    global main_config, problem_config, prompt_context_parts, llm_config
    logger = get_logger("PromptLLMConfig")
    try:
    ...
    try:
        ...
        if not prompt_context_parts or not isinstance(prompt_context_parts, dict):
            ...
            # Not critical to raise here, as prompt generation might still work with minimal context
    except FileNotFoundError:
        logger.warning("Prompt context file %s not found. Prompt generation might be less effective." % prompt_context_path)
        prompt_context_parts = {} 
    except yaml.YAMLError as e:
        logger.warning("Error parsing prompt context YAML %s: %s. Prompt generation might be less effective." % (prompt_context_path, e))
        prompt_context_parts = {}
...
try:
    ...
    get_logger("PromptLLMTemplate").warning("Jinja2 FileSystemLoader for 'templates/' failed: %s. Prompting will fail if template not found." % e)
    template_env = None

def _initialize_prompt_templates():
    global prompt_templates, template_env
    logger = get_logger("PromptLLMTemplate")
...
async def _get_llm_completion(prompt_text: str, client: httpx.AsyncClient) -> Tuple[Optional[str], str]:
    ...
    logger = get_logger("PromptLLMCompletion")
    ...
    try:
        system_message_content = ("You are a helpful assistant for generating clear and effective prompts for a Python coding LLM. "
                                  "Follow the user's instructions carefully and precisely. "
                                  "Output only the requested prompt string or delegation requests.")
...
def _extract_prompt_string(response_text: str) -> Optional[str]:
    logger = get_logger("PromptLLMParser")
    if not response_text:
        return None
    
    # If delegation tags are present, it's not a final prompt string.
    if "<delegate_subtask>" in response_text: 
        logger.debug("Delegation tags found in response, not extracting as direct prompt string.")
        return None
    
    # Look for a specific markdown block for the prompt string
    match = re.search(r"^```prompt\s*\n(.*?)\n```$", response_text, re.DOTALL | re.MULTILINE)
    if match:
        extracted_prompt = match.group(1).strip()
        logger.debug("Extracted prompt string from markdown fence.")
        return extracted_prompt
    
    # Fallback: If no markdown fences, assume the entire cleaned response is the prompt.
    ...
    if not cleaned_fallback:
        logger.warning("No prompt string could be extracted. Response was likely empty or only non-prompt elements after cleaning.")
        return None
    
    logger.debug("Extracted prompt string using fallback (assuming entire response is prompt after cleaning).")
...
async def generate_prompt_variant(
    ...
    global prompt_templates, prompt_context_parts, problem_config, llm_config
    logger = get_logger("PromptLLMGenerator")
    ...
    if prompt_template_to_use is None:
        logger.critical("Prompt template '%s' is not available. Cannot generate prompt." % current_prompt_template_key)
        return None, "PROMPT_TEMPLATE_ERROR"

    # --- Self-correction loop for prompt generation ---
    # This loop handles both initial generation and self-correction attempts
    current_prompt_content = context_from_evolution_loop.get("parent_prompt")
    current_error_feedback = context_from_evolution_loop.get("previous_error_feedback")
    current_prompt_for_db = "N/A (Initial Call)" # To track the prompt that led to the final version
    ...
    for attempt in range(max_correction_attempts + 1 if enable_self_correction else 1): # +1 for initial attempt
        if attempt > 0: # Only log for actual correction attempts
            logger.info("Prompt self-correction attempt %s/%s..." % (attempt, max_correction_attempts))
            # Update context for self-correction
            context_from_evolution_loop["parent_prompt"] = current_prompt_content
        ...
        if hierarchical_enabled and current_delegation_depth == 0:
            ...
            for iteration in range(max_iterations):
                logger.info("Prompt Generation/Orchestration iteration %s/%s" % (iteration + 1, max_iterations))
                
                template_render_context = {
                    "problem": problem_config,
                    "prompt_ctx": prompt_context_parts,
                    "parent_prompt": context_from_evolution_loop.get("parent_prompt"),
                ...
                prompt_template_to_use = prompt_templates.get(current_prompt_template_key)
                if prompt_template_to_use is None:
                    logger.error("Prompt template '%s' became unavailable mid-loop." % current_prompt_template_key)
                    return None, "PROMPT_TEMPLATE_ERROR_UNEXPECTED"

                try:
                    prompt_text = prompt_template_to_use.render(template_render_context)
                except Exception as e:
                    logger.error("Error rendering prompt template '%s': %s" % (current_prompt_template_key,e))
                ...
                if delegation_requests and current_delegation_depth < max_depth:
                    ...
                    for req in delegation_requests:
                        ...
                        sub_task_futures.append(
                            llm_tools.generate_delegated_task_content( # Updated function call
                                req,
                                client,
                                current_prompt_string=context_from_evolution_loop.get("parent_prompt", ""), # Pass parent prompt string
                                prompt_templates_ref=prompt_templates,
                                ...
                                extract_content_func=_extract_prompt_string # Use prompt string extraction
                            )
                        )
                    ...
                    for req, (sub_content, _sub_prompt) in zip(delegation_requests, sub_task_results_tuples):
                        new_delegation_results.append({
                            ...
                            "expected_signature": req["expected_signature"], # This might need to be adapted for prompts
                    ...
                    if iteration == max_iterations - 1:
                        logger.warning("Max delegation iterations reached. LLM must integrate or provide final prompt.")
                    continue
                
                # If no delegation, assume it's final prompt string
                logger.info("No further delegation. Attempting to extract final prompt string.")
                generated_prompt = _extract_prompt_string(llm_response)
                if generated_prompt:
                    logger.log(VERBOSE_LEVEL_NUM, "Final Prompt Output (Iteration: %s):\nPrompt (first 300 chars):\n%s...\nFinal Prompt:\n%s",
                               iteration + 1, actual_prompt_used[:300], generated_prompt)
                    current_prompt_content = generated_prompt # Update for self-correction loop
                    current_prompt_for_db = actual_prompt_used
                    break # Exit orchestration loop, found a prompt
                else:
                    logger.warning("LLM did not delegate, nor provided extractable prompt string. Output: %s" % llm_response[:200])
                    if iteration == max_iterations - 1:
                        logger.error("Max iterations reached in main loop without extractable final prompt.")
                        current_prompt_content = None # Indicate failure
                        current_prompt_for_db = actual_prompt_used
                        break
                    current_error_feedback = "LLM did not provide extractable prompt string or delegation. Please try again." # Feedback for next iteration
            else: # If orchestration loop finishes without breaking (max_iterations reached)
                logger.error("Prompt Generation/Orchestration loop finished (max_iterations) without producing final prompt.")
                current_prompt_content = None # Indicate failure
                # current_prompt_for_db is already set from last iteration

        else: # --- Standard (Non-Hierarchical) Prompt Generation ---
            template_render_context = {
                "problem": problem_config,
                "prompt_ctx": prompt_context_parts,
                "parent_prompt": context_from_evolution_loop.get("parent_prompt"),
            ...
            current_prompt_for_db = actual_prompt_used # Log the prompt that led to this version

            if not llm_response_content:
                current_prompt_content = None
                break # Exit self-correction loop
            
            generated_prompt = _extract_prompt_string(llm_response_content)
            if not generated_prompt:
                logger.warning("Failed to extract prompt from LLM response. Response (first 500 chars): %s" % llm_response_content[:500])
                current_error_feedback = "LLM did not provide extractable prompt string. Please try again."
                current_prompt_content = None # Indicate failure for this attempt
            else:
                current_prompt_content = generated_prompt
                break # Exit self-correction loop, found a prompt

        if current_prompt_content is not None: # If a prompt was successfully generated (or corrected)
            return current_prompt_content, current_prompt_for_db
        elif attempt == max_correction_attempts: # If all attempts failed
            logger.error("Prompt generation failed after %s attempts." % (attempt + 1))
            return None, current_prompt_for_db # Return None to indicate failure

    return None, current_prompt_for_db # Should not be reached if loops are structured correctly
...
async def _test_main():
    ...
    logger = get_logger("PromptLLMTest")
    logger.info("Prompt LLM Generator Standalone Test Started.")
    ...
    example_context = {
        'parent_prompt': "Initial prompt for a sorting function.",
        'previous_error_feedback': None
    }
    
    async with httpx.AsyncClient() as client:
        logger.info("\n--- Test: Generating prompt (hierarchical: %s) ---" % 
                    (llm_config.get('enable_hierarchical_generation')))
        prompt_str, prompt_used = await generate_prompt_variant(example_context, client, current_delegation_depth=0)
        
        print("\nFinal Prompt Used (for last LLM call in the chain):")
        print(prompt_used)
        print("\nGenerated Prompt (final integrated output, if any):")
        print(prompt_str if prompt_str else "No final prompt generated/extracted.")

    logger.info("Prompt LLM Generator Standalone Test Finished.")
